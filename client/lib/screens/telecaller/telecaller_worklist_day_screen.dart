...
/// Worklist for one specific date (opened by tapping a day tile on
/// `TelecallerAllWorklistScreen`'s week grid). Same search / rich card UI as
/// Today Worklist, scoped to whichever accounts fire on `date` per the beat
/// plan, instead of "today".
class TelecallerWorklistDayScreen extends StatefulWidget {
  final String date;  // YYYY-MM-DD
  final String label; // short weekday label, e.g. "Mon" — for the AppBar title

  const TelecallerWorklistDayScreen({super.key, required this.date, required this.label});

  @override
  State<TelecallerWorklistDayScreen> createState() => _TelecallerWorklistDayScreenState();
}

class _TelecallerWorklistDayScreenState extends State<TelecallerWorklistDayScreen> {
  bool _loading = true;
  List<Map<String, dynamic>> _items = [];
  // account_id's actually scheduled on `widget.date` — the scope for this screen.
  Set<String> _dayIds = {};
  ...
  Future<void> _load() async {
    ...
    final dayF       = ApiService.getWeekBeatPlan(date: widget.date);
    final callbacksF = ApiService.getTelecallerCallbacks();
    final items      = await worklistF;
    final dayRes     = await dayF;
    ...
    // Accounts actually firing on this date — this is the scope for the screen.
    final dayRaw = dayRes['data'];
    if (dayRaw is List) {
      for (final t in dayRaw) {
    ...
    // Follow-ups — mark due/overdue ones so the pill + filter chip still work.
  ...
  bool _inScope(Map<String, dynamic> w) => _dayIds.contains('${w['account_id']}');
  ...
  List<Map<String, dynamic>> get _visible {
    final list = _items.where(_inScope).where(_matchesFilter).where(_matchesSearch).toList();
  ...
  int _countFor(String key) {
    final old = _filter;
    _filter = key;
    final n = _items.where(_inScope).where(_matchesFilter).where(_matchesSearch).length;
    _filter = old;
    return n;
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text('${widget.label} Worklist', style: const TextStyle(fontWeight: FontWeight.w700)),
            Text('$_dateLabel  •  ${_items.where(_inScope).length} accounts',
                style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.w500, color: Colors.white70)),
          ],
        ),
              ...
              children: [
                ...
                Expanded(
                  child: visible.isEmpty
                      ? Center(
                          child: Text(
                            _items.where(_inScope).isEmpty
                                ? 'No accounts scheduled on ${widget.label}'
                ...
                child: Builder(builder: (_) {
                  ...
                  return Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: selected ? accent : const Color(0xFFE7E7E7), width: 1.4),
                    ),
                    child: Text(
                      '${f.$2} (${_countFor(f.$1)})',
                      style: TextStyle(
