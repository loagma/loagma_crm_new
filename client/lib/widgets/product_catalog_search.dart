...
class ProductCatalogSearch extends StatefulWidget {
  ...
  const ProductCatalogSearch({super.key, required this.onAdd, this.secondaryHeader});
      ...
      children: [
        TextField(
          ...
          decoration: InputDecoration(
            ...
            prefixIcon: const Icon(Icons.search_rounded, color: kGoldDark, size: 20),
            isDense: true,
            filled: true,
            fillColor: const Color(0xFFFAFAFA),
            contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12),
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: kGold.withValues(alpha: 0.5))),
            enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: kGold.withValues(alpha: 0.5))),
            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: kGold, width: 1.4)),
          ),
        ),
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('Could not load products.', style: TextStyle(fontSize: 12.5, color: Colors.grey.shade500)),
                          const SizedBox(height: 8),
                          TextButton.icon(
                            onPressed: () => _search(_searchCtrl.text),
                            icon: const Icon(Icons.refresh_rounded, size: 16, color: kGoldDark),
                            label: const Text('Retry', style: TextStyle(color: kGoldDark)),
                          ),
                        ],
                      ),
                    )
...
class _ProductCatalogCard extends StatefulWidget {
  ...
  final void Function(OrderLineItem item) onAdd;
  const _ProductCatalogCard({super.key, required this.product, required this.onAdd});
...
class _ProductCatalogCardState extends State<_ProductCatalogCard> {
  ...
  void initState() {
    super.initState();
    _packs = ((widget.product['packs'] as List?) ?? const []).cast<Map<String, dynamic>>();
    final defaultPack = _findPack((p) => p['is_default'] == true) ?? (_packs.isNotEmpty ? _packs.first : null);
  ...
  Map<String, dynamic>? get _selectedPack => _findPack((p) => p['id'] == _selectedPackId);
  ...
  Widget _singlePackBox(Map<String, dynamic> pack) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(11), border: Border.all(color: const Color(0xFFE7E7E7))),
      child: Row(children: [
        Text('$label: ', style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600, color: Color(0xFF20242B))),
        if (mrp > price) ...[
          Text('₹${mrp.toStringAsFixed(0)}', style: TextStyle(fontSize: 11.5, decoration: TextDecoration.lineThrough, color: Colors.grey.shade400)),
          const SizedBox(width: 4),
        ],
        Text('₹${price.toStringAsFixed(price == price.roundToDouble() ? 0 : 2)}', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: kGoldDark)),
      ]),
    );
  }
  ...
  Widget _packSummaryLine() {
    ...
    return Text.rich(TextSpan(children: [
      TextSpan(text: '$label: ', style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600, color: Color(0xFF20242B))),
      TextSpan(text: '₹${price.toStringAsFixed(price == price.roundToDouble() ? 0 : 2)}', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: kGoldDark)),
    ]));
  }
  ...
  Widget _qtyStepper() => Container(
        decoration: BoxDecoration(color: const Color(0xFFF6F6F7), borderRadius: BorderRadius.circular(20)),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          GestureDetector(
            onTap: () => setState(() => _qty = _qty > 1 ? _qty - 1 : 1),
            child: const Padding(padding: EdgeInsets.all(8), child: Icon(Icons.remove_rounded, size: 15, color: Color(0xFF20242B))),
          ),
          SizedBox(
            width: 22,
            child: Text('$_qty', textAlign: TextAlign.center, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
          ),
          GestureDetector(
            onTap: () => setState(() => _qty += 1),
            child: const Padding(padding: EdgeInsets.all(8), child: Icon(Icons.add_rounded, size: 15, color: Color(0xFF20242B))),
          ),
        ]),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: const Color(0xFF141F1F).withValues(alpha: 0.04), blurRadius: 10, offset: const Offset(0, 3))],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(
              width: 52, height: 52,
              decoration: BoxDecoration(color: const Color(0xFFF6F6F7), borderRadius: BorderRadius.circular(11)),
              ...
              child: Icon(Icons.inventory_2_outlined, color: Colors.grey.shade400, size: 22),
            ),
          ...
          if (_packs.length == 1)
            ...
            Container(
              ...
              child: Row(children: [
                ...
                Text('No price listed for this vendor', style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
              ]),
            ),
          const SizedBox(height: 10),
          Row(children: [
            Opacity(opacity: _canAdd ? 1 : 0.4, child: IgnorePointer(ignoring: !_canAdd, child: _qtyStepper())),
            const Spacer(),
            GestureDetector(
              onTap: _canAdd ? _add : null,
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 10),
                decoration: BoxDecoration(color: _canAdd ? kGold : Colors.grey.shade300, borderRadius: BorderRadius.circular(20)),
                child: const Text('Add', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: Colors.white)),
              ),
            ),
          ]),
        ],
