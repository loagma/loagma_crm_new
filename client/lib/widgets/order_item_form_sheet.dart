...
Future<Map<String, dynamic>?> showOrderItemFormSheet(
  ...
  return showModalBottomSheet<Map<String, dynamic>>(
    ...
    builder: (_) =>
        _OrderItemFormSheet(initial: initial, itemNumber: itemNumber),
  );
}
...
class _OrderItemFormSheetState extends State<_OrderItemFormSheet> {
  ...
  double get _tax => _gstPercent > 0
      ? _productTotal - _productTotal / (1 + _gstPercent / 100)
      : 0;
  ...
  Future<void> _pickProduct() async {
    ...
    if (picked != null) {
      setState(() {
        _productCtrl.text = picked.product.text.trim();
        _productId = picked.productId;
        _hsnCode = picked.hsnCode;
        _gstPercent = picked.gstPercent;
        // The catalog already knows a real qty/unit/price for this pack —
        // carry them over instead of leaving the form's previous values
        // (or the "1 / PCS / 0" defaults) sitting there mismatched.
        _qtyCtrl.text = picked.qty.text;
        _unit = picked.unit;
        if (!_units.contains(_unit)) _units = [..._units, _unit];
        _priceCtrl.text = picked.unitPrice.text;
      });
      picked.dispose();
    }
  }
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'Item ${widget.itemNumber}  |  HSN: ${_hsnCode?.isNotEmpty == true ? _hsnCode : 'NA'}',
                    style: TextStyle(
                      fontSize: 11,
                      fontWeight: FontWeight.w700,
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            _label(
                              _gstPercent > 0
                                  ? 'Total Tax (${_gstPercent.toStringAsFixed(_gstPercent % 1 == 0 ? 0 : 2)}%)'
                                  : 'Total Tax',
                            ),
                            Text(
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              _productTotal.toStringAsFixed(2),
                              style: const TextStyle(
                                fontSize: 13.5,
